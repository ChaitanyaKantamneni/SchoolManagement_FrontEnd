...
/**
 * Class Responsibility: Handles view logic and user interactions for StopsComponent.
 */
export class StopsComponent extends BasePermissionComponent {
  ...
  FetchAcademicYearCount(isSearch: boolean) {
    ...
    return this.apiurl.post<any>('Tbl_Stops_CRUD_Operations', payload);
  }
  ...
  /**
   * Executes the operation: FetchAllRoutesList
   * Parameters: none
   * Rationale: Standard operational controller for the active view.
   */
  FetchAllRoutesList() {
    ...
    this.apiurl.post<any>('Tbl_Route_CRUD_Operations', requestData)
      .subscribe(
        (response: any) => {
          if (response && Array.isArray(response.data)) {
            this.AllRoutesList = response.data.map((item: any) => {
              const isActiveString = item.isActive === "1" ? "Active" : "InActive";
              return {
                ID: item.id,
                Name: item.name
