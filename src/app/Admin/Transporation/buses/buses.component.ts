...
/**
 * Class Responsibility: Handles view logic and user interactions for BusesComponent.
 */
export class BusesComponent extends BasePermissionComponent {
  ...
  SyllabusForm: any = new FormGroup({
    ...
    AssistantMobNo: new FormControl('', [Validators.required, Validators.pattern(/^[0-9]{10}$/)]),
    ...
    DistanceCostPerKM: new FormControl('', [Validators.required, Validators.pattern(/^[0-9]+(\.[0-9]+)?$/)]),
  ...
  FetchAcademicYearCount(isSearch: boolean) {
    ...
    return this.apiurl.post<any>('Tbl_Bus_CRUD_Operations', payload);
  }
  ...
  SubmitSyllabus(){
    if(this.SyllabusForm.invalid){
      ...
      const data = {
        Name: (this.SyllabusForm.get('Name')?.value ?? '').toString().trim(),
        RegNo: (this.SyllabusForm.get('RegNo')?.value ?? '').toString().trim(),
        Driver: (this.SyllabusForm.get('Driver')?.value ?? '').toString().trim(),
        AssistantName: (this.SyllabusForm.get('AssistantName')?.value ?? '').toString().trim(),
        AssistantMobNo: (this.SyllabusForm.get('AssistantMobNo')?.value ?? '').toString().trim(),
        ...
        DistanceCostPerKM: (this.SyllabusForm.get('DistanceCostPerKM')?.value ?? '').toString().trim(),
        MaxCapacity: (this.SyllabusForm.get('MaxCapacity')?.value ?? '').toString().trim(),
