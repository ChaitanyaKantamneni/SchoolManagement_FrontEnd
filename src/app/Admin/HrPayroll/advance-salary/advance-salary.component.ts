import { CommonModule } from '@angular/common';
import { Component, OnInit } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { MatIconModule } from '@angular/material/icon';
import { DashboardTopNavComponent } from '../../../SignInAndSignUp/dashboard-top-nav/dashboard-top-nav.component';
import { ApiServiceService } from '../../../Services/api-service.service';
import { LoaderService } from '../../../Services/loader.service';

@Component({
  selector: 'app-advance-salary',
  standalone: true,
  imports: [CommonModule, FormsModule, MatIconModule, DashboardTopNavComponent],
  templateUrl: './advance-salary.component.html',
  styleUrl: './advance-salary.component.css'
})
export class AdvanceSalaryComponent implements OnInit {
  constructor(private apiurl: ApiServiceService, public loader: LoaderService) {}

  IsAddNewClicked = false;
  isViewModalOpen = false;
  searchQuery = '';
  editIndex: number | null = null;
  viewRow: any = null;
  schoolList: any[] = [];
  academicYearList: any[] = [];
  selectedSchoolID = '';
  selectedAcademicYearID = '';

  staffList: string[] = ['Select', 'Priyanka R', 'Anu A S', 'Arvind S'];

  form = {
    staffName: 'Select',
    amount: null as number | null,
    date: ''
  };

  records: Array<{ staffName: string; amount: number; date: string }> = [];

  get filteredRecords(): typeof this.records {
    const q = this.searchQuery.trim().toLowerCase();
    if (!q) return this.records;
    return this.records.filter((r) => r.staffName.toLowerCase().includes(q));
  }

  AddNewClicked(): void {
    this.IsAddNewClicked = !this.IsAddNewClicked;
    if (this.IsAddNewClicked) {
      this.editIndex = null;
      this.form = { staffName: 'Select', amount: null, date: '' };
    }
  }

  get isAdmin(): boolean {
    const role = sessionStorage.getItem('RollID') || localStorage.getItem('RollID');
    return role === '1';
  }

  ngOnInit(): void {
    if (this.isAdmin) {
      this.FetchSchoolsList();
    } else {
      this.selectedSchoolID =
        sessionStorage.getItem('SchoolID') ||
        sessionStorage.getItem('schoolID') ||
        sessionStorage.getItem('schoolId') ||
        '';
      this.FetchAcademicYearsList();
    }
  }

  FetchSchoolsList() {
    this.loader.show();
    this.apiurl.post<any>('Tbl_SchoolDetails_CRUD', { Flag: '2' }).subscribe({
      next: (response: any) => {
        this.schoolList = Array.isArray(response?.data)
          ? response.data.map((item: any) => ({ ID: item.id, Name: item.name }))
          : [];
        this.loader.hide();
      },
      error: () => {
        this.schoolList = [];
        this.loader.hide();
      }
    });
  }

  FetchAcademicYearsList() {
    this.loader.show();
    this.apiurl
      .post<any>('Tbl_AcademicYear_CRUD_Operations', { SchoolID: this.selectedSchoolID || '', Flag: '2' })
      .subscribe({
        next: (response: any) => {
          this.academicYearList = Array.isArray(response?.data)
            ? response.data.map((item: any) => ({ ID: item.id, Name: item.name }))
            : [];
          this.loader.hide();
        },
        error: () => {
          this.academicYearList = [];
          this.loader.hide();
        }
      });
  }

  onAdminSchoolChange(event: Event) {
    const target = event.target as HTMLSelectElement;
    this.selectedSchoolID = target.value === '0' ? '' : target.value;
    this.selectedAcademicYearID = '';
    this.FetchAcademicYearsList();
  }

  addAdvanceSalary(): void {
    if ((this.isAdmin && !this.selectedSchoolID) || !this.selectedAcademicYearID) return;
    if (this.form.staffName === 'Select' || !this.form.amount || !this.form.date) return;

    const payload = {
      staffName: this.form.staffName,
      amount: this.form.amount,
      date: this.form.date
    };
    if (this.editIndex !== null) {
      this.records[this.editIndex] = payload;
    } else {
      this.records.unshift(payload);
    }

    this.IsAddNewClicked = false;
    this.editIndex = null;
    this.form = { staffName: 'Select', amount: null, date: '' };
  }

  editReview(row: any, index: number): void {
    this.editIndex = index;
    this.form = {
      staffName: row.staffName,
      amount: row.amount,
      date: row.date
    };
    this.IsAddNewClicked = true;
  }

  viewReview(row: any): void {
    this.viewRow = row;
    this.isViewModalOpen = true;
  }

  closeViewModal(): void {
    this.isViewModalOpen = false;
    this.viewRow = null;
  }
}
